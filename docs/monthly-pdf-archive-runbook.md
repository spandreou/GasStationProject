...
checks pass. The live BP Kallis deployment may set this flag to `true` only
after the full rollout checklist below is complete.
...
the BP Kallis origin only. CORS configuration does not make archive files
public; rules must still require admin access for read and write.
